...
use crate::wasm::structure::{WasmModule, SectionType};
use log::{debug, info};
...
fn count_funcs(module: &WasmModule) -> usize {
    if let Some(code_section) = module.sections.iter().find(|s| s.section_type == SectionType::Code) {
...
fn find_safe_split_points(func_body: &[u8]) -> Vec<usize> {
    ...
    while pos < func_body.len() {
        ...
        match instr_type {
            ...
            WasmInstrType::BlockEnd => if block_depth > 0 { block_depth -= 1 },
            _ => {}
        }
        ...
        if stack_depth == 0 && block_depth == 0 && 
        ...
        match opcode {
            ...
            0x20..=0x24 => { // Local variable and global variable index - LEB128 encoded
                while pos < func_body.len() && (func_body[pos] & 0x80) != 0 {
                    pos += 1;
                }
                if pos < func_body.len() {
            ...
            0x10 => { // call - function index, LEB128 encoded
                while pos < func_body.len() && (func_body[pos] & 0x80) != 0 {
                    pos += 1;
                }
                if pos < func_body.len() {
...
fn find_large_functions(module: &WasmModule, exported_funcs: &HashSet<usize>) -> Result<Vec<usize>> {
    ...
    for (func_idx, (func_pos, func_size)) in func_bodies.iter().enumerate() {
        if *func_size > 100 { // The threshold for large functions
            ...
            if let Some(code_section) = module.sections.iter().find(|s| s.section_type == SectionType::Code) {
                ...
                if safe_points.len() >= 1 {
                    debug!("Function {} with size {} has {} safe split points, adding to list", 
                          func_idx, func_size, safe_points.len());
                    large_funcs.push(func_idx);
                } else {
                    debug!("Function {} has size {} but only {} safe split points, skipping", 
                          func_idx, func_size, safe_points.len());
                }
            }
        }
    ...
    debug!("Found {} large functions suitable for splitting", large_funcs.len());
    Ok(large_funcs)
}
...
fn split_function(module: WasmModule, func_idx: usize) -> Result<WasmModule> {
    ...
    let function_section_idx = function_section_idx.ok_or_else(|| anyhow!("Missing Function section"))?;
    ...
    
    {
        ...
        if func_idx >= num_func_types {
            return Err(anyhow!("Function index {} out of range (only {} functions)", func_idx, num_func_types));
        }
        ...
        if pos < function_data.len() {
            ...
            return Err(anyhow!("Function type index not found for function {}", func_idx));
        }
        ...
        if num_funcs <= func_idx {
            return Err(anyhow!("Function index {} out of range (only {} functions)", func_idx, num_funcs));
        }
    ...
    if split_points.is_empty() {
        debug!("Not enough safe split points found for function {}. Skipping split.", func_idx);
        return Ok(modified_module);
    }
    
    let num_subfuncs = split_points.len() + 1; // The number of split points + 1 = the number of sub-functions
    
    debug!("Splitting function {} into {} sub-functions at safe points", func_idx, num_subfuncs);
    ...
    // Update the code section, replace the original function and add sub-functions
    {
        ...
        code_section.data.splice(func_start..func_body_end, complete_main_body.iter().cloned());
