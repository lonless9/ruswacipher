...
fn scramble_local_name(data: &mut Vec<u8>, pos: usize) {
    ...
    match data[pos] {
        // 局部变量指令
        0x20 | 0x21 | 0x22 | 0x23 => { // get_local, set_local, tee_local instructions
            // 只处理单字节变量索引以确保安全
            if pos + 1 < data.len() && (data[pos + 1] & 0x80) == 0 {
                // 生成新的变量索引，确保在有效范围内
                // 为了更安全，我们只使用原始索引值或更小的值
                data[pos + 1] = rng.random_range(0..=std::cmp::min(original_var_index, max_local_var)) as u8;
            }
        },
        _ => {} // 不处理其他类型的指令
    }
