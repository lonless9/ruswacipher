...
use crate::wasm::structure::{WasmModule, SectionType, Section};
...
pub fn find_virtualizable_functions(module: &WasmModule) -> Result<Vec<usize>> {
    ...
    debug!("Found {} suitable functions for virtualization", virtualizable_funcs.len());
...
pub fn virtualize_functions(module: WasmModule) -> Result<WasmModule> {
    ...
    info!("Found {} functions suitable for virtualization", virtualizable_funcs.len());
    ...
    info!("Virtualization protection completed: {} functions virtualized", virtualized_count);
    Ok(modified_module)
}
...
fn virtualize_function(module: WasmModule, func_idx: usize) -> Result<WasmModule> {
    ...
    replace_with_vm_interpreter(&mut modified_module, func_idx, encrypted_bytecode, vm_metadata)?;
...
fn convert_to_vm_bytecode(func_body: &[u8]) -> Result<Vec<u8>> {
    ...
    debug!("开始将{}字节的WebAssembly指令转换为VM字节码", func_body.len());
    ...
    debug!("Conversion completed: {} bytes of VM bytecode generated", vm_bytecode.len());
    Ok(vm_bytecode)
}
...
fn encrypt_vm_bytecode(vm_bytecode: &[u8], vm_metadata: &[u8]) -> Result<Vec<u8>> {
    ...
    let key_start = if vm_metadata.len() >= 16 { vm_metadata.len() - 16 } else { 0 };
    ...
    debug!("Encrypted {} bytes of VM bytecode, resulting in {} bytes of encrypted data", 
           vm_bytecode.len(), encrypted_bytecode.len());
...
fn replace_with_vm_interpreter(
    ...
    debug!("Function {} successfully replaced with VM interpreter", func_idx);
    Ok(())
}
...
fn generate_vm_interpreter(encrypted_bytecode: &[u8], vm_metadata: &[u8]) -> Result<Vec<u8>> {
    ...
    debug!("生成VM解释器代码 - 加密字节码大小: {} 字节", encrypted_bytecode.len());
    ...
    debug!("Generated {} bytes of VM interpreter code", interpreter_code.len());
