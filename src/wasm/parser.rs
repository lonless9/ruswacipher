...
use super::structure::{WasmModule, Section, SectionType};
...
fn process_payload(module: &mut WasmModule, payload: &Payload, data: &[u8]) -> Result<()> {
    match payload {
        ...
        Payload::CodeSectionStart { count: _, range, .. } => {
...
fn add_section_from_range(module: &mut WasmModule, range: &std::ops::Range<usize>, section_type: SectionType, data: &[u8]) {
