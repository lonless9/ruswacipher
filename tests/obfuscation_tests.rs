...
use ruswacipher::obfuscation::{
    ...
    split_large_functions, virtualize_functions, find_virtualizable_functions
};
...
fn test_variable_renaming() {
    ...
    let has_code_section = reparsed_module.sections.iter()
...
fn test_dead_code_insertion() {
    ...
    let obfuscated_code_size = obfuscated_module.sections.iter()
        .find(|section| section.section_type == ruswacipher::wasm::structure::SectionType::Code)
        .map(|section| section.data.len())
        .unwrap_or(0);
...
fn test_function_splitting() {
    ...
    let has_function_section = reparsed_module.sections.iter()
...
fn test_find_virtualizable_functions() {
    ...
    println!("Found {} virtualizable functions", virtualizable_funcs.len());
}
...
fn test_virtualization() {
    ...
    if wasm_bytes.len() > 0 {
        println!("Debug: First few bytes: {:?}", &wasm_bytes[..std::cmp::min(10, wasm_bytes.len())]);
    }
    
    // Add more detailed debug information
    println!("Debug: WASM sections:");
    for (i, section) in obfuscated_module.sections.iter().enumerate() {
        println!("  Section {}: Type {:?}, Size {} bytes", 
                 i, section.section_type, section.data.len());
    }
