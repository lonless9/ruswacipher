use ruswacipher::obfuscation::control_flow::{
    obfuscate_control_flow, add_dead_code
};
...
fn test_find_function_bodies() {
    ...
    // 确保代码段存在
    let has_code_section = obfuscated_module.sections.iter()
...
fn test_dead_code_addition() {
    ...
    // 获取原始代码大小
    let original_code_section = module.sections.iter()
        .find(|section| section.section_type == SectionType::Code)
        .expect("代码段不存在");
    ...
    // 获取混淆后的代码大小
    let obfuscated_code_section = obfuscated_module.sections.iter()
        .find(|section| section.section_type == SectionType::Code)
        .expect("混淆后代码段不存在");
    ...
    // 验证代码段大小增加
    println!("原始代码大小: {}, 混淆后: {}", original_code_size, obfuscated_code_size);
...
fn test_control_flow_obfuscation() {
    ...
    // 检查混淆后的控制流指令数量
    let obfuscated_code_section = get_code_section_data(&reparsed_module).expect("混淆后代码段不存在");
    let obfuscated_control_flow_count = find_control_flow_instructions(obfuscated_code_section).len();
    
    // 控制流混淆可能增加或改变控制流指令，但不会移除所有指令
    println!("原始控制流指令数量: {}, 混淆后: {}", 
...
fn test_combined_control_flow_obfuscation() {
    ...
    // 获取原始代码大小
    let original_code_size = module.sections.iter()
        .find(|section| section.section_type == SectionType::Code)
