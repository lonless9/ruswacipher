...
use ruswacipher::wasm::{self, structure::{WasmModule, Section, SectionType}};
...
fn test_parse_and_load_wasm() -> Result<()> {
    ...
    assert!(!module.sections.is_empty(), "WASM module should have sections");
...
fn test_custom_sections() -> Result<()> {
    ...
    assert_eq!(custom_sections[0].data, custom_data, "Custom section data should match");
    ...
    let custom_sections = module.get_custom_sections("test_section");
    assert!(custom_sections.is_empty(), "Custom section should be removed");
...
fn test_add_section() -> Result<()> {
    ...
    assert_eq!(module.sections[0].section_type, SectionType::Custom, "Section type should be Custom");
    assert_eq!(module.sections[0].name.as_ref().unwrap(), "test_section", "Section name should match");
    assert_eq!(module.sections[0].data, vec![1, 2, 3, 4], "Section data should match");
...
fn test_write_and_read_roundtrip() -> Result<()> {
    ...
    assert_eq!(custom_sections.len(), 1, "Should have one custom section after roundtrip");
    assert_eq!(custom_sections[0].data, vec![1, 2, 3, 4], "Custom section data should match after roundtrip");
...
fn test_custom_section_in_real_wasm() -> Result<()> {
    ...
    assert_eq!(custom_sections.len(), 1, "Should have one custom section");
    assert_eq!(custom_sections[0].data, section_data, "Custom section data should match");
    ...
    assert_eq!(custom_sections.len(), 1, "Should still have the custom section after serialization");
    assert_eq!(custom_sections[0].data, section_data, "Custom section data should still match after serialization");
